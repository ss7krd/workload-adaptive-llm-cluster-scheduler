...
exhaustive = [6, 14, 36, 53, 70, 72, 73, 74, 75, 78, 88, 99, 100]#cdf ##(a good result for SLO=0.16s, for llama)
exhaustive = [2, 3, 6, 21, 28, 43, 71, 90, 91, 91, 91, 95, 100] #(a good result for SLO=0.25s, 90% reqs were within the SLO., for llama)
exhaustive = [9, 14, 20, 40, 52, 70, 81, 88, 95, 97, 99, 100, 100]
   
ax.plot(stepX,np.array(exhaustive)/100,c=color_list[0],ls='-',label="Exhaustive",fillstyle='none', linewidth = 4)

sarathi = [4, 1, 17, 11, 8, 16, 6, 5, 6, 16, 3, 7, 1]
sarathi = [4, 9, 15, 19, 23, 31, 39, 48, 61, 78, 83, 94, 100]
sarathi = [2, 2, 3, 7, 10, 20, 25, 40, 48, 59, 70, 82, 100]
...
llumnix = [9, 10, 22, 33, 41, 57, 63, 68, 75, 91, 94, 100, 100] ##(a good result for SLO=0.16s, for llama)
llumnix = [9, 10, 22, 33, 41, 47, 53, 55, 70, 94, 100, 100, 100] #(a good result for SLO=0.25s, 90% reqs were within the SLO for exhaustive., for llama)
llumnix = [8, 12, 20, 31, 42, 45, 52, 59, 68, 85, 90, 100, 100]
# ax.plot(stepX, np.array(static)-5, c = 'blue', marker = 'o', markersize = 26, ls = '-', label = "Scrooge", fillstyle = 'full', linewidth = 4)
...
ax.set_xticklabels([0.75, 1.75, 2.75, 3.75])
# ax.set_ylim(0,4500)
